-- =====================================================
-- 聊天室系统数据库 - 建库文件
-- 运行方式：psql -U postgres -f init.sql
-- =====================================================
...
-- COMMENT ON COLUMN users.status IS '用户状态：online-在线 offline-离线 away-离开 busy-忙碌';
...
CREATE INDEX idx_offline_messages_user ON offline_messages(user_id, created_at);
...
CREATE TABLE conversations (
    id BIGSERIAL PRIMARY KEY,
    uuid UUID NOT NULL DEFAULT uuid_generate_v4(), -- 新增 uuid 列
    user_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
