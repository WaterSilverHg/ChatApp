...
//    }
    ...
    if (url.contains("/api/auth/login")) {
    ...
    else if (url.contains("/api/friends") && !url.contains("/remark") && !url.contains("/group") && !url.contains("/block") && !url.contains("/unblock")) {
        ...
        QJsonArray messages = doc.isArray() ? doc.array() :
                              (obj.contains("content") ? QJsonDocument::fromJson(obj["content"].toString().toUtf8()).array() : QJsonArray());
        emit groupMessagesPageReceived(messages);
    }
...
{
    sendAuthenticatedRequest("GET", "/api/friends", QJsonObject());
}
...
void HttpApiClient::getPrivateMessages(const QString& uid)
{
    sendAuthenticatedRequest("GET", QString("/api/messages/private/%1").arg(uid), QJsonObject());
}
...
void HttpApiClient::getGroupMessages(const QString& gid)
{
    sendAuthenticatedRequest("GET", QString("/api/messages/group/%1").arg(gid), QJsonObject());
}
...
{
    sendAuthenticatedRequest("GET", "/api/groups", QJsonObject());
}
